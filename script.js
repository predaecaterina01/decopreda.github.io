...
const dismissLoader = (() => {
  ...
  return function dismiss() {
    ...
    /* ── Resetăm overflow DOAR dacă nav-ul nu este deschis ── */
...
function updateParallax() {
  ...
  if (sy <= window.innerHeight) {
    introScene.style.transform =
      `translateY(${clamp(sy * 0.12, 0, window.innerHeight * 0.15).toFixed(2)}px)`;
  }
}
...
let _cursorRAF = null, cursorVisible = false;
...
/* Ascunde cursorul custom pe touch devices */
...
$$('[data-magnetic]').forEach(btn => {
  ...
  function tick() {
    ...
    const done = !active && Math.abs(cx - tx) < T && Math.abs(cy - ty) < T;
    if (done) { btn.style.transform = ''; cx = 0; cy = 0; rAF = null; }
...
function setError(inp, b) {
  const w = inp && inp.closest('.f');
  if (w) w.classList.toggle('f--error', b);
}

function showSuccess(m) {
  if (!formOkEl) return;
  formOkEl.textContent = m;
  formOkEl.classList.add('show');
  setTimeout(() => { formOkEl.textContent = ''; formOkEl.classList.remove('show'); }, 9000);
}

function showFormErr(m) {
  if (!formErrEl) return;
  formErrEl.textContent = m;
  formErrEl.classList.add('show');
  setTimeout(() => { formErrEl.textContent = ''; formErrEl.classList.remove('show'); }, 10000);
}

function setSub(btn, lbl, loading) {
  if (!btn || !lbl) return;
...
if (commissionForm) {
  commissionForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const n  = $('#f-name',  commissionForm);
    const em = $('#f-email', commissionForm);
    const st = $('#f-studio',commissionForm);
    const ty = $('#f-type',  commissionForm);
    const br = $('#f-brief', commissionForm);
    const sb = $('button[type="submit"]', commissionForm);
    const sl = sb && $('.btn-submit__label', sb);
    ...
    try {
      const res = await fetch(FORMSPREE, {
        ...
        body: JSON.stringify({
          name:         n.value.trim(),
          email:        em.value.trim(),
          studio:       st?.value.trim() || '—',
          project_type: ty.value,
          message:      br.value.trim(),
      ...
      if (res.ok) {
        ...
        setSub(sb, sl, false);
        showSuccess('Your commission request has been received. The studio will respond within 24 hours.');
      } else {
        throw new Error(data?.errors?.map(e => e.message).join(', ') || 'Submission failed.');
      }
    } catch (err) {
      console.error('[Deco Preda]', err);
      setSub(sb, sl, false);
      showFormErr('Could not send message. Please write to studio@decopreda.com');
    }
  });
}
...
const PC = {
  RATE: 40,
  BULK: 0.15,
  MAT:  { microcement: 1, resin: 1.1, stone: 0.95, 'resin-design': 1.2 },
  SYS:  { basic: 0.85, medium: 1, premium: 1.35 },
  SYS_LBL: { basic: 'Basic coat', medium: 'Medium', premium: 'Premium' }
};

function fmtRON(n) { return n.toLocaleString('ro-RO', { maximumFractionDigits: 0 }) + ' RON'; }
function easeOutCubic(t) { return 1 - Math.pow(1 - t, 3); }

function rollUp(el, target, dur = 700) {
...
function computeEst(mat, sqm, sys) {
  const mm = PC.MAT[mat] ?? 1, sm = PC.SYS[sys] ?? 1;
  const gross = Math.round(sqm * PC.RATE * mm * sm);
  let dp = 0;
  if (sqm >= 200) dp = PC.BULK;
  else if (sqm > 100) dp = PC.BULK * ((sqm - 100) / 100);
  const sav = Math.round(gross * dp), base = gross - sav;
  return { base, saving: sav, discountPct: dp, isBulk: dp > 0, low: Math.round(base * .9), high: Math.round(base * 1.35) };
}

function renderResult(el, sqm, mat, sys) {
  const { base, saving, discountPct, isBulk, low, high } = computeEst(mat, sqm, sys);
  const sl = PC.SYS_LBL[sys] || sys, pl = Math.round(discountPct * 100);
  let h = `<span class="pcalc__price">0 RON</span><span class="pcalc__amount-unit">${sqm} m² · ${sl}</span>`;
  if (isBulk) h += `<span class="pcalc__bulk-badge">−${pl}% volume discount · saving ${fmtRON(saving)}</span>`;
  h += `<p class="pcalc__range">Indicative range: ${fmtRON(low)} – ${fmtRON(high)}</p>`;
  h += `<p class="pcalc__rate">${PC.RATE} RON/m² base${isBulk ? ' · volume rate applied' : ''}</p>`;
  el.innerHTML = h;
  const pe = el.querySelector('.pcalc__price');
  if (pe) rollUp(pe, base);
}
...
function toggleDrawer(btn, drawer) {
  ...
  if (open) {
    ...
    const mat = drawer.dataset.material;
    const inp = drawer.querySelector('.pcalc__input');
    const sel = drawer.querySelector('.pcalc__select');
    if (inp && parseFloat(inp.value) > 0) {
      const r = drawer.querySelector('.pcalc__result');
      if (r) renderResult(r, parseFloat(inp.value), mat, sel?.value);
    } else if (inp) {
...
function resetDrawer(drawer) {
  const inp = drawer.querySelector('.pcalc__input');
  const r   = drawer.querySelector('.pcalc__result');
  if (r && r.querySelector('.pcalc__price')) {
    r.style.transition = 'opacity .35s ease';
    r.style.opacity = '0';
    setTimeout(() => {
      if (inp) { inp.value = '00'; inp.blur(); }
      r.innerHTML = '<span class="pcalc__neutral">—</span>';
      r.style.opacity = '0';
    ...
    if (inp) { inp.value = '00'; inp.blur(); }
    if (r) r.innerHTML = '<span class="pcalc__neutral">—</span>';
  }
}

function handleInput(drawer) {
  const mat = drawer.dataset.material;
  const inp = drawer.querySelector('.pcalc__input');
  const sel = drawer.querySelector('.pcalc__select');
  const r   = drawer.querySelector('.pcalc__result');
  if (!inp || !sel || !r) return;
  const sqm = parseFloat(inp.value.trim());
  if (!inp.value.trim() || inp.value === '00' || isNaN(sqm) || sqm <= 0) {
    r.innerHTML = '<span class="pcalc__neutral">—</span>';
    return;
  }
  renderResult(r, sqm, mat, sel.value);
}
...
document.querySelectorAll('.panel__estimate-btn').forEach(btn => {
  ...
  if (inp) {
    inp.addEventListener('input',   () => handleInput(drawer));
    inp.addEventListener('keydown', e  => { if (e.key === 'Enter') { handleInput(drawer); inp.blur(); } });
    inp.addEventListener('focus',   ()  => { if (!inp.value.trim() || inp.value === '00' || inp.value === '0') inp.value = ''; });
  ...
  if (sel) sel.addEventListener('change', () => handleInput(drawer));
});

document.querySelectorAll('.panel').forEach(panel => {
  let t = null;
  panel.addEventListener('mouseleave', () => {
    const d = panel.querySelector('.panel__calc');
    if (!d) return;
    t = setTimeout(() => resetDrawer(d), 120);
  });
...
(function () {
  document.querySelectorAll('.panel__surface[data-carousel]').forEach(surface => {
    ...
    function go(idx) {
      slides[cur].classList.remove('pcs__slide--active');
      cur = (idx + slides.length) % slides.length;
      slides[cur].classList.add('pcs__slide--active');
      if (dotsWrap) dotsWrap.querySelectorAll('.pcs__dot').forEach((d, i) =>
        d.classList.toggle('pcs__dot--active', i === cur));
    }

    function start() { timer = setInterval(() => go(cur + 1), D); }
    function stop()  { clearInterval(timer); }

    btnPrev && btnPrev.addEventListener('click', () => { stop(); go(cur - 1); start(); });
    btnNext && btnNext.addEventListener('click', () => { stop(); go(cur + 1); start(); });

    let tsx = 0;
    surface.addEventListener('touchstart', e => { tsx = e.changedTouches[0].clientX; }, { passive: true });
    surface.addEventListener('touchend',   e => {
      const dx = e.changedTouches[0].clientX - tsx;
      if (Math.abs(dx) > 40) { stop(); go(dx < 0 ? cur + 1 : cur - 1); start(); }
    }, { passive: true });

    surface.addEventListener('mouseenter', stop);
    surface.addEventListener('mouseleave', start);

    surface.addEventListener('carousel:reload', () => {
      stop();
      const ns = Array.from(surface.querySelectorAll('.pcs__slide'));
      if (!ns.length) return;
      ns.forEach((s, i) => s.classList.toggle('pcs__slide--active', i === 0));
      if (dotsWrap) {
        dotsWrap.innerHTML = '';
        ns.forEach((_, i) => {
          const d = document.createElement('button');
...
(async function () {
  try {
    ...
    console.info('[Deco Preda] images.json not found.');
  }
})();
...
(function () {
  ...
  const grid        = document.querySelector('.systems__panels');
  if (!panels.length) return;
  if (grid) grid.style.position = 'relative';

  const COPY = {
    industrial:    { subline: '2 systems specified for your environment.', sealText: 'View Architectural & Living Systems',  mobileMoreText: '+ 2 more systems' },
    architectural: { subline: 'Infinite application.',                     sealText: 'View Industrial & Logistics Systems', mobileMoreText: '+ 2 more systems' }
  };

  let sealEl = null, mobileMoreEl = null;
  ...
  function revealVeiled() {
    panels.forEach(p => { p.classList.remove('is-veiled'); p.classList.add('is-revealed'); });
    if (sealEl)      sealEl.classList.add('is-hidden');
    if (mobileMoreEl) mobileMoreEl.classList.add('is-hidden');
    if (grid) grid.classList.remove('filtered-2', 'filtered-3');
  }

  function filterByDomain(domain) {
    cleanup();
    panels.forEach(p => { p.classList.remove('is-hidden', 'is-veiled', 'is-revealed'); p.style.display = ''; });
    if (heading)  heading.innerHTML = COPY[domain].subline;
    if (moreWrap) moreWrap.style.display = 'none';

    const sec = [];
    panels.forEach(p => {
      if (!(p.dataset.domain === 'both' || p.dataset.domain === domain)) {
        p.classList.add('is-veiled');
        sec.push(p);
      }
    });

    panels.forEach(p => { if (!p.classList.contains('is-veiled')) grid.insertBefore(p, grid.firstChild); });
    panels.forEach(p => { if (p.classList.contains('is-veiled'))  grid.appendChild(p); });

    if (!sec.length || !grid) return;
    ...
    grid.appendChild(sealEl);
    sealEl.querySelector('.systems__seal-btn').addEventListener('click', revealVeiled);
    requestAnimationFrame(() => posSeal());

    const fv = grid.querySelector('.panel.is-veiled');
    mobileMoreEl = document.createElement('div');
    mobileMoreEl.className = 'systems__mobile-more';
    mobileMoreEl.innerHTML = `<button class="systems__mobile-more-btn" type="button"><span>${COPY[domain].mobileMoreText}</span><span aria-hidden="true">↓</span></button>`;
    if (fv) grid.insertBefore(mobileMoreEl, fv);
    else    grid.appendChild(mobileMoreEl);

    mobileMoreEl.querySelector('.systems__mobile-more-btn').addEventListener('click', () => {
      sec.forEach(p => { p.style.display = ''; p.classList.remove('is-veiled'); p.classList.add('is-revealed'); });
      mobileMoreEl.classList.add('is-hidden');
    });
  }

  function posSeal() {
    if (!sealEl || !grid) return;
    const vp = panels.filter(p => p.classList.contains('is-veiled') && !p.classList.contains('is-hidden'));
    if (!vp.length) return;
    const f = vp[0], l = vp[vp.length - 1];
    const imgH = clamp(220, 200, 320);
    sealEl.style.cssText = `position:absolute;left:${(f.offsetLeft + l.offsetLeft + l.offsetWidth) / 2}px;top:${f.offsetTop + imgH * .5}px;transform:translate(-50%,-50%);z-index:10;pointer-events:auto;`;
  }

  window.addEventListener('resize', () => {
    if (sealEl && !sealEl.classList.contains('is-hidden')) requestAnimationFrame(posSeal);
  });

  if (btnInd) btnInd.addEventListener('click', (e) => {
    e.preventDefault();
  ...
  if (btnArch) btnArch.addEventListener('click', (e) => {
    e.preventDefault();
